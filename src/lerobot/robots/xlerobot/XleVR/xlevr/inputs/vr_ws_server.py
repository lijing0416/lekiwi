...
class VRControllerState:
    ...
    def __init__(self, hand: str):
        ...
        # SO101 mapping uses VR +X as pitch and VR +Y(back) as tool roll.
        self.z_axis_rotation = 0.0  # Kept for compatibility; stores VR Y-axis roll.
...
class VRWebSocketServer(BaseInputProvider):
    ...
    async def process_controller_data(self, data: Dict):
        ...
        # 检查右手柄的摇杆和按钮状态
        for hand in [f'{ACTIVE_CONTROLLER_HAND}Controller']:
            if hand in data:
        ...
        # Process only the right controller for single-arm SO101 control.
        active_controller_key = f"{ACTIVE_CONTROLLER_HAND}Controller"
        if active_controller_key in data:
            await self.process_single_controller(ACTIVE_CONTROLLER_HAND, data[active_controller_key])
        ...
        if position and all(k in position for k in ['x', 'y', 'z']):
            ...
            goal = ControlGoal(
                ...
                metadata={
                    ...
                    "coordinate_frame": "vr_controller",
    ...
    def extract_roll_from_quaternion(self, current_quat: np.ndarray, origin_quat: np.ndarray) -> float:
        """Extract SO101 wrist roll around the VR Y/back axis."""
        ...
        try:
            ...
            # Project the relative rotation onto VR +Y(back), the SO101 tool-roll axis.
